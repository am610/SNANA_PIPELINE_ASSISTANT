...
- 2026-09-10 — (Antigravity) Implemented Cross-Agent Skills + MCP Architecture per direction document:
  - Created canonical, model-neutral Agent Skill at `agent-skill/snana-assistant/SKILL.md` with progressive disclosure references in `references/` (`debugging-order.md`, `diagnosis-output-contract.md`, `safety-and-scope.md`). Enforces on-demand BM25 retrieval rather than loading full entries corpus into model context.
  - Implemented centralized safety boundary policy in `src/snana_assistant/tool_policy.py` (500 KB / 1000 line limits, binary/archive/FITS rejection, no recursive symlink traversal, strict suppression of environment tokens/secrets).
  - Built read-only Model Context Protocol (FastMCP) server in `src/snana_assistant/mcp_server.py` and registered entry point `snana-assistant-mcp` in `pyproject.toml`. Exposes 9 bounded read-only tools: `search_knowledge`, `search_manual`, `check_job_status`, `read_log_tail`, `diff_config`, `list_directory`, `search_files`, `read_text_file`, `inspect_snana_environment`.
  - Added packaging adapters: Claude Code marketplace + plugin (`integrations/claude-marketplace/`), OpenAI Codex install helper (`integrations/codex/install-skill.sh` and `README.md`), and Google Gemini CLI skill/MCP registration docs (`integrations/gemini/README.md`).
  - Added sync and integrity tools: `scripts/sync_agent_skill.py` (keeps backward-compatible `skill/SKILL.md` and packaged plugins synced with canonical source) and `scripts/verify_integrations.py` (checks schemas, manifests, and drift).
  - Created deterministic MCP test harness `eval/test_mcp_tools.py` (9 passed, 100%) and cross-agent contract test cases `eval/agent_contract_cases.yaml`.
  - Updated `README.md`, `CHANGELOG.md`, and `ROADMAP.md`.
